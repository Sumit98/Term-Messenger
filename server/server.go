...
func (s *server) Send(ctx context.Context, t *pb.Text) (*pb.Ack, error) {
	ch = append(ch, *t)
	log.Println("In Send: From:", t.Msg.From, "To:", t.Msg.To, "Message:", t.Msg.Message)
...
func (s *server) Receive(ctx context.Context, usr *pb.Str) (*pb.Text, error) {
	for i, txt := range ch {
		if txt.Msg.To == usr.Noti {
			log.Println("From:", txt.Msg.From, "To:", txt.Msg.To, "Message:", txt.Msg.Message)
			ch[i] = ch[len(ch)-1]
			ch[len(ch)-1] = pb.Text{}
			ch = ch[:len(ch)-1]
			return &txt, nil
	...
	return &pb.Text{Msg: &pb.TextMail{From: "", To: "", Message: ""}}, nil
}
